...
import { symmetric_tsp_parser } from './symmetric_tsp_parser.mjs';

// In this example we are solving the Symmetric Travelling Salesperson Problem (TSP)
// using the coordinates set from file a280.tsp (See 
// http://comopt.ifi.uni-heidelberg.de/software/TSPLIB95/ for more details about 
// TSPLIB instances):
const filename = "examples/problems/symmetric_tsp/att48.tsp";
const start_line = 7;
const end_line = 54;

// Now we read the file, get the coordinates as places and solve the problem with GA:
symmetric_tsp_parser(filename, start_line, end_line).then(places => {
    ///////// Solution //////////
    ...
    // Then, we can tell the fitness model to use the pseudoeuclidean distance equation:
    om.set_fitness_config(f_id, "distance", distance.PSEUDOEUCLIDEAN);
